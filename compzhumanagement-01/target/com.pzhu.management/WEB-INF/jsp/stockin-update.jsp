...
<div class="content">
<form action="/stockIn/updateStockinByKey?sid=${stt.sid}" method="post">
<table>
		<tr>
            <td class="title">订&nbsp;单&nbsp;编&nbsp;号</td> 
            <td class="edit"><input
	type="text" class="txt" name="cname" value="${stt.sid}" readonly="readonly"></td></tr>
<tr>
            <td class="title">客&nbsp;户&nbsp;名&nbsp;称</td> 
            <td class="edit">
<select name="customerid.cid"  id="clientname">
<c:forEach items="${Ocu}" var="Oc">
    <option value="${Oc.cid}" <c:if test="${Oc.cid eq stt.customerid.cid}">selected</c:if> >${Oc.cname}</option>
 </c:forEach>
</select></td></tr>
<tr>
...
<c:forEach items="${Om}" var="Om1">
    <option value="${Om1.mid}" <c:if test="${Om1.mid eq stt.merchandiseid.mid}">selected</c:if> >${Om1.mname}</option>
 </c:forEach>
</select></td></tr>
<tr>
...
<c:forEach items="${Oe}" var="Oe1">
    <option value="${Oe1.eid}" <c:if test="${Oe1.eid eq stt.employeeid.eid}">selected</c:if> >${Oe1.ename}</option>
 </c:forEach>
</select></td></tr>
<tr>
            <td class="title">商&nbsp;品&nbsp;数&nbsp;量</td> 
            <td class="edit"><input type="text" class="txt"
	name="amount" value="${stt.amount}"></td></tr>
<tr>
            <td class="title">商&nbsp;品&nbsp;价&nbsp;格</td> 
            <td class="edit"><input type="text" class="txt"
	name="sprice" value="${stt.sprice}"></td></tr>
<tr>
            <td class="title">进&nbsp;货&nbsp;时&nbsp;间</td> 
            <td class="edit"><input type="text" class="txt"
	name="stockindate" value="${stt.stockindate}"  onClick="WdatePicker()"></td></tr></table><br>
